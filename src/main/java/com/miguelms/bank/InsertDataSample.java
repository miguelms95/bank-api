package com.miguelms.bank;

import com.miguelms.bank.model.*;
import com.miguelms.bank.repository.*;
...
public class InsertDataSample {
    ...
    public void insertData() {
        ...
        atmMachineRepository.saveAll(Arrays.asList(atm1, atm2));
        ...
        Card c1 = new Card().setId(1l).setActive(false).setCardType(CardType.DEBIT).setBankAccount(b1).setUser(u1)
                .setCardNumber("11112222333344445555").setPin(Utils.encryptMd5("1234")).setCardLimit(3000);
        Card c1_2 = new Card().setActive(true).setCardType(CardType.CREDIT).setBankAccount(b1).setUser(u1)
                .setCardNumber("12112222333344445555").setPin(Utils.encryptMd5("1234")).setCardLimit(3000);
        Card c2 = new Card().setActive(true).setCardType(CardType.DEBIT).setBankAccount(b2).setUser(u2)
                .setCardNumber("21112222333344445555").setPin(Utils.encryptMd5("1234")).setCardLimit(3000);
        Card c3 = new Card().setActive(true).setCardType(CardType.DEBIT).setBankAccount(b4).setUser(u3)
                .setCardNumber("31112222333344445555").setPin(Utils.encryptMd5("1234")).setCardLimit(3000);
        Card c4 = new Card().setActive(true).setCardType(CardType.DEBIT).setBankAccount(b1).setUser(u4)
                .setCardNumber("41112222333344445555").setPin(Utils.encryptMd5("4321")).setCardLimit(3000);

        cardRepository.saveAll(Arrays.asList(c1, c1_2, c2, c3, c4));
        ...
        activityRepository.saveAll(Arrays.asList(ac1, ac2, ac3, ac4, ac5, ac6));
